...
from data import get_mnist_iid, get_mnist_imbalance,get_mnist_taxi_area,get_mnist_dirichlet,initial_mnist, update_training_subset,get_dataloader_by_indices, initial_training_subset, get_cifar10_iid, get_cifar10_imbalance,get_cifar10_dirichlet,get_cifar100_iid, get_cifar100_imbalance,get_cifar100_dirichlet,get_fashionmnist_iid, get_fashionmnist_imbalance,get_fashionmnist_taxi_area,get_fashionmnist_dirichlet,get_harbox_iid, get_harbox_imbalance,get_harbox_dirichlet
...
def Decentralized_Cache_mixing_process(suffix_dir,train_loader,test_loader,num_round,local_ep):
    model = []
    local_cache = []
    acc_global = []
    acc_global_before_aggregation = []
    ...
    for i in range(num_round):
        ...
        if i<args.give_up_cache_round:
            ...
            print('Updated/aggregated model time/combination:')
            for index in range(num_car):
                model[index] = cache_average_process_mixing(model[index],index,local_cache[index],weights)
        else:
            receiver_buffer = {}
            model_before_training = copy.deepcopy(model)
            for seconds in range(args.epoch_time):
        ...
        with open(model_dir+'/log.txt','a') as file:
            ...
            for index in range(num_car):
                ...
                file.write(str(class_acc_list[index][-1])+'\n')
                file.write('Local Cache model version:'+'\n')
                for key in range(len(local_cache[index])):
                    file.write(str(key)+':'+str(local_cache[index][key]['time'])+'\n')#,local_cache[index][key]['fresh_metric'])
            file.write('----------------------------------------------------------------------'+'\n')
            file.write('Average test acc:'+str(np.average(acc_global,axis=0)[-1])+'\n')
            file.write('Variance test acc:'+str(np.var(acc_global,axis=0)[-1])+'\n')
...
def Decentralized_Cache_balance_process(suffix_dir,train_loader,test_loader,num_round,local_ep):
    model = []
    local_cache = []
    acc_global = []
    acc_global_before_aggregation = []
    ...
    # current_model_combination_table = np.eye(num_car)#statistic_data#
    current_class_test = np.zeros([num_car,10])
    model_dir = './result/'+str(date_time.strftime('%Y-%m-%d %H_%M_%S'))+'_'+task+'_'+data_distribution+'_'+str(Randomseed)+'_'+args.algorithm+'_'+str(cache_size)+'_local_ep_'+str(local_ep)+'_epoch_time_'+str(args.epoch_time)+'_kick_out_'+str(args.kick_out)+'_balance_ratio_'+str(args.balance_ratio) +suffix_dir
    pair,area = write_info(model_dir)
    # current_model_distribution = copy.deepcopy(statistic_data)
    if distribution == 'by_area':
        train_loader = []
        train_indices = []
        for index in range(num_car):
    ...
    for i in range(num_round):
        ...
        if i<args.give_up_cache_round:
            ...
            for seconds in range(args.epoch_time):
                for a,b in pair[i*args.epoch_time+seconds]: 
                    # update_model_cache(local_cache, model_before_training[a],model_before_training[b],a,b,i, cache_size, kick_out)
                    update_model_cache_distribution(local_cache, model_before_training[a],model_before_training[b],a,b,i, cache_size, args.kick_out,statistic_data,max_std,args.balance_ratio)
            #########################
            #Statistic cache age and cache number
            cache_age = 0
            cache_num = 0
            ...
            for index in range(num_car):
                model[index] = cache_average_process(model[index],index,i,local_cache[index],weights)
                # model[index],current_model_distribution[index] = cache_average_process_distribution(model[index],index,i,local_cache[index],weights,statistic_data[index])
        else:
            receiver_buffer = {}
            model_before_training = copy.deepcopy(model)
            for seconds in range(args.epoch_time):
        ...
        with open(model_dir+'/log.txt','a') as file:
            ...
            file.write('Average test acc:'+str(np.average(acc_global,axis=0)[-1])+'\n')
            file.write('Variance test acc:'+str(np.var(acc_global,axis=0)[-1])+'\n')
        with open(model_dir+'/average_acc_'+task+'_'+data_distribution+'_'+str(Randomseed)+'_'+args.algorithm+'_'+str(cache_size)+'_local_ep_'+str(local_ep)+'_epoch_time_'+str(args.epoch_time)+'_kick_out_'+str(args.kick_out)+'_balance_ratio_'+str(args.balance_ratio)+suffix_dir+'.txt','a') as file:
           file.write(str(i)+':'+str(np.average(acc_global,axis=0)[-1])+'\t'+str(np.var(acc_global,axis=0)[-1])+'\n')
...
def Decentralized_Cache_random_process(suffix_dir,train_loader,test_loader,num_round,local_ep):
    model = []
    local_cache = []
    acc_global = []
    acc_global_before_aggregation = []
    ...
    for i in range(num_round):
        ...
        if i<args.give_up_cache_round:
            ...
            for seconds in range(args.epoch_time):
                for a,b in pair[i*args.epoch_time+seconds]: 
                    # update_model_cache(local_cache, model_before_training[a],model_before_training[b],a,b,i, cache_size, kick_out)
                    update_model_cache_random(local_cache, model_before_training[a],model_before_training[b],a,b,i, cache_size)
            #########################
            #Statistic cache age and cache number
            cache_age = 0
            cache_num = 0
        ...
        with open(model_dir+'/log.txt','a') as file:
            ...
            file.write('Average test acc:'+str(np.average(acc_global,axis=0)[-1])+'\n')
            file.write('Variance test acc:'+str(np.var(acc_global,axis=0)[-1])+'\n')
        with open(model_dir+'/average_acc_'+task+'_'+data_distribution+'_'+str(Randomseed)+'_'+args.algorithm+'_'+str(cache_size)+'_local_ep_'+str(local_ep)+'_epoch_time_'+str(args.epoch_time)+'_kick_out_'+str(args.kick_out)+'.txt','a') as file:
           file.write(str(i)+':'+str(np.average(acc_global,axis=0)[-1])+'\t'+str(np.var(acc_global,axis=0)[-1])+'\n')
...
#     # current_model_combination_table = np.eye(num_car)#statistic_data#
#     current_class_test = np.zeros([num_car,10])
#     model_dir = './result/'+str(date_time.strftime('%Y-%m-%d %H_%M_%S'))+'_'+task+'_'+data_distribution+'_'+str(Randomseed)+'_'+args.algorithm+'_'+str(cache_size)+'_local_ep_'+str(local_ep)+suffix_dir
#     pair,area = write_info(model_dir)
...
#         if i<args.give_up_cache_round:
#             for a,b in pair[i]: 
#                 cache_info_dynamic  = update_model_cache_global(local_cache,  model[a],model[b],a,b,i,cache_size, cache_info_dynamic ,args.kick_out)
#             if kick_out == True:
#                 for index in range(num_car):
#                     local_cache[index], cache_info_dynamic  = kick_out_timeout_model_cache_info(local_cache[index],i-args.kick_out, cache_info_dynamic )
...
#             file.write('Average test acc:'+str(np.average(acc_global,axis=0)[-1])+'\n')
#             file.write('Variance test acc:'+str(np.var(acc_global,axis=0)[-1])+'\n')
#         with open(model_dir+'/average_acc_'+task+'_'+data_distribution+'_'+str(Randomseed)+'_'+args.algorithm+'_'+str(cache_size)+'_local_ep_'+str(local_ep)+suffix_dir+'.txt','a') as file:
#            file.write(str(i)+':'+str(np.average(acc_global,axis=0)[-1])+'\t'+str(np.var(acc_global,axis=0)[-1])+'\n')
...
def Decentralized_Cache_plus_process(suffix_dir,train_loader,test_loader,num_round,local_ep):
    ...
    for i in range(num_round):
        ...
        for a,b in pair[i]: 
            ...
            model[b].to(device)
            if i<args.give_up_cache_round:
                update_model_cache_plus(local_cache, model[a],model[b],temp_weights[a],temp_weights[b],a,b,i, cache_size, kick_out)
        if kick_out == True:
            for index in range(num_car):
                local_cache[index] = kick_out_timeout_model(local_cache[index],i-args.kick_out)
        ...
        if i<args.give_up_cache_round:
            for index in range(num_car):
                model[index] = cache_average_process_plus(model[index],index,local_cache[index],temp_weights)
...
if __name__ == '__main__':
    ...
    if rank ==0:
        ...
        elif args.algorithm == 'cache_balance':
            dfl_loss, dfl_acc_global, df_class_acc, dfl_acc_local, model_dir = Decentralized_Cache_balance_process(suffix,train_loader, test_loader,num_round,local_ep)
        elif args.algorithm == 'cache_random':
            dfl_loss, dfl_acc_global, df_class_acc, dfl_acc_local, model_dir = Decentralized_Cache_random_process(suffix,train_loader, test_loader,num_round,local_ep)
        ...
        elif args.algorithm == 'cache_mixing':
            dfl_loss, dfl_acc_global, df_class_acc, dfl_acc_local, model_dir = Decentralized_Cache_mixing_process(suffix,train_loader, test_loader,num_round,local_ep)
        else:
            raise ValueError('Error')
        if args.algorithm == 'ml':
            print(acc)
